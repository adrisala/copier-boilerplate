...
if git rev-parse --verify copier-boilerplate-sync >/dev/null 2>&1; then
    ...
    git checkout -b copier-boilerplate-sync --track origin/devel
fi
# At this point stop the script if any of the following commands fail for any reason
set -e
if [[ $APPROVE == true ]]; then
    copier recopy --defaults --skip-answered
else
    copier recopy
fi
...
git push --force -o merge_request.create -o merge_request.target=devel -o merge_request.description="This PR was created by automated script" origin copier-boilerplate-sync